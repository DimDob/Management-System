...
class FeaturesControllerIntegrationTest extends IntegrationTest {
    ...
    def "Test if feature is correctly attached to an epic when created"() {
        given:
        Feature feature = new Feature(name: "First feature", description: "Some feature", status: "COMPLETED", owner: "labaduba@abv.bg", assignee: "labaduba@abv.bg")
        ...
        expectedEpic.features.get(0).getAssignee() == feature.getAssignee()
    }

    def "Test if an exception is thrown when a feature is requested to be saved into an epic which does not exist"() {
        given:
        Feature feature = new Feature(name: "First feature", description: "Some feature", status: "COMPLETED", owner: "Unknown", assignee: "Unknown")
    ...
    def "Test if showFeatures() returns all features in the database"() {
        given:
        Feature feature1 = new Feature(name: "First feature", description: "Some feature", status: "COMPLETED", owner: "Unknown", assignee: "Unknown")
        Feature feature2 = new Feature(name: "Second feature", description: "Some feature2", status: "COMPLETED", owner: "Unknown2", assignee: "Unknown")
    ...
    def "Test if findFeatureByName() returns the proper feature from the passed epic"() {
        ...
        expectedFeature.getAssignee() == feature.getAssignee()
    ...
    def "Test if updateFeature() is correctly updating a feature of an existing epic"() {
        ...
        Feature updatedFeature = new Feature(name: "First feature", description: "Some feature update", status: "COMPLETED", owner: "Unknown", assignee: "Unknown")
        ...
        epicFeature.getAssignee() == updatedFeature.getAssignee()

    }

    def "Test if updateFeature() will throw an exception if the requested epic does not exist"() {
        when:
        featuresController.updateFeature(UUID.randomUUID(), new Feature(name: "First feature", description: "Some feature update", status: "COMPLETED", owner: "Unknown", assignee: "Unknown"))
        then:
        thrown(IllegalArgumentException)
    }
    ...
    def "Test if patchFeature() will patch a field of an existing feature entity of an existing epic"() {
        ...
        expectedPatchedFeature.getAssignee() == feature.getAssignee()
    ...
    def "Test if patchFeature() will throw an exception if the patched feature does not exist"() {
        ...
        Feature feature = new Feature(name: "First feature", description: "Random", owner: "Random", status: "COMPLETED", assignee: "Unknown")
        Map<Object, Object> fields = [name       : "First feature",
                                      description: "Some patched description",
                                      status     : "COMPLETED",
                                      owner      : "Unknown",
