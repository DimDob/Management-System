...
class EpicsControllerIntegrationTest extends IntegrationTest {
    ...
    def "Test if saving epic in the database is successful"() {
        given:
        Epic epic = new Epic(epicId: UUID.randomUUID(), name: "Test epic")
    ...
    def "Test one() if a saved epic is returned when passed it's id as an argument"() {
        ...
        then:
        def expectedEpic = new Epic(epicId: lastEpicInDatabase.getEpicId(), name: lastEpicInDatabase.getName())
    ...
    def "Test updateEpic() if the epic is saved in the database & then successfully updated"() {
        ...
        epicsController.updateEpic(lastEpic.getEpicId(), new Epic(epicId: lastEpic.getEpicId(), name: "Some updated name"))
    ...
    def "Test updateEpic() if entity is not present"() {
        when:
        epicsController.updateEpic(UUID.randomUUID(), new Epic(epicId: UUID.randomUUID(), name: "New name"))
        then:
        thrown(IllegalArgumentException)
